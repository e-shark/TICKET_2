...
fin:  
    ...
    $emechanics = Employee::find()->orderBy('lastname', 'firstname', 'patronymic')
            ->where(['occupation_id' => 3])->asArray()->all(); 
    $emechanicList = ArrayHelper::map($emechanics, 'id','lastname'); 
  ?>
  <?php $form = ActiveForm::begin(['id'=>'formarrr', 'action'=>['sendstreetap','id'=>'id'], 'method'=>"post"]); ?>
    <h4> <b>
        ...
        <?= $form->field($model, 'emechanic',//Электромеханик 
            [ 'template' => '<div class=col-md-2> 
            <label> <span>Электромеханик:</span>{input}</label>{error}</div>',
            ])->widget(Select2Widget::className(),
                [ 'options' => [
                    'name'=>'emechanic',
                    'prompt'=>'Все',
        ...
        <?= Html::a('Сбросить', ['/employeeeq/employee/append','id'=>$model->id],
