...
<div class="row">
  ...
  <br>
      ...
      <div class="row" >
        ...
        <div class="col-md-4" id="divExecutantDispDepsList">
          <?php echo Html::dropDownList('tiDispDepSelect','null',ArrayHelper::map($model->tiDispDepsList,'id','divisionname'),['id'=>'tiDispDepSelect','class'=>'form-control']); ?>
        </div>
...
function CheckForLasNeeded(){
  if (1 == $("#ObjectsSelect").val()) {
    ...
    console.log($("#ObjectsSelect").val());

    if ("004" == $("#ObjectsSelect").val()){
      // объект - Диспетчеризация
      $("#divVDESExecutantLas").hide();
      $("#divVDESExecutantDepsList").hide();
      $("#divExecutantDispDepsList").show();
    }else{
      $("#divExecutantDispDepsList").hide();
